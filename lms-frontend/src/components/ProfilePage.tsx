...
export default function ProfilePage({ sidebarSections }: ProfilePageProps) {
  ...
  return (
    <DashboardLayout sidebarSections={sidebarSections}>
      <h1 className="text-3xl font-bold text-text mb-1">Profile</h1> {/* page heading */}
      <p className="text-muted mb-8">Manage your account details and password.</p>

      {/* ACCOUNT INFO — read-only summary */}
      <div className="bg-surface rounded-2xl p-6 shadow-soft mb-6 max-w-lg"> {/* glass card, matches the rest of the theme */}
        <div className="flex items-center gap-4"> {/* avatar + basic info row */}
          <div className="w-14 h-14 rounded-full bg-gradient-to-br from-primary-500 to-secondary-400 flex items-center justify-center text-white text-xl font-semibold"> {/* larger version of the navbar avatar */}
            {user?.name?.charAt(0).toUpperCase() ?? '?'}
          </div>
          <div>
            <p className="text-text font-medium">{user?.name ?? 'Loading...'}</p> {/* current name */}
            <p className="text-sm text-muted">{user?.email}</p> {/* email, not editable here */}
            <span className="inline-block mt-1 text-xs px-2 py-0.5 rounded-full bg-primary-500/10 text-primary-700 border border-primary-500/20 capitalize"> {/* role badge */}
              {user?.role}
            </span>
          </div>
        </div>
      ...
      {/* EDIT NAME FORM */}
      <form onSubmit={handleNameSubmit} className="bg-surface rounded-2xl p-6 shadow-soft mb-6 max-w-lg"> {/* glass form card */}
        <h2 className="font-medium text-text mb-4">Display name</h2>
      ...
      <form onSubmit={handlePasswordSubmit} className="bg-surface rounded-2xl p-6 shadow-soft max-w-lg"> {/* glass form card */}
