    ...
    <header className="h-16 bg-background/70 backdrop-blur-xl border-b border-border flex items-center justify-between px-8 sticky top-0 z-10">
      ...
      <div className="flex items-center gap-5">
        ...
        <button
          ...
          className="relative text-muted hover:text-text transition"
          aria-label="Cart"
        >
          <svg
            className="w-5 h-5"
        ...
        >
          <div className="w-8 h-8 rounded-full bg-gradient-to-br from-primary-500 to-secondary-400 flex items-center justify-center text-white text-xs font-semibold group-hover:scale-105 transition">
            {user?.name?.charAt(0).toUpperCase() ?? '?'}
          </div>

          <span className="text-sm font-medium text-muted-dark group-hover:text-text transition">
            {user?.name ?? 'Loading...'}
          </span>
        </button>
