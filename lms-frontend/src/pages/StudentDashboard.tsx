...
export default function StudentDashboard() {
  ...
  return (
    <DashboardLayout sidebarSections={studentSidebarSections}>
      ...
      <p className="text-muted mb-8">Pick up where you left off, or find something new to learn.</p>
      ...
      <div className="relative max-w-md mb-10">
        ...
        <input
          ...
          className="w-full bg-surface-strong border border-border rounded-full pl-11 pr-5 py-3 text-sm text-text placeholder:text-placeholder outline-none focus:border-primary-500/50 focus:ring-4 focus:ring-primary-500/10 transition"
        />
        {coursesQuery.isFetching && !isInitialLoad && (
          <div className="absolute right-4 top-1/2 -translate-y-1/2 w-4 h-4 border-2 border-primary-500/30 border-t-primary-400 rounded-full animate-spin" />
        )}
      </div>
      ...
      {!isInitialLoad && courses?.length === 0 && (
        <div className="text-center py-20">
          ...
          <p className="text-text font-medium mb-1">{search ? 'No courses match your search' : 'No courses yet'}</p>
